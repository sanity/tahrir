...
public class TrSessionImpl implements TrSession {
	...
	public TrRemoteConnection connection(final PhysicalNetworkLocation physicalLocation) {
		return connection(new RemoteNodeAddress(physicalLocation, null), false);
	}

	public TrRemoteConnection connection(final PhysicalNetworkLocation physicalLocation, final boolean unilateral) {
		return connection(new RemoteNodeAddress(physicalLocation, null), unilateral);
	}

	public TrRemoteConnection connection(final RemoteNodeAddress address) {
		return connection(address, false);
	}

	public TrRemoteConnection connection(final RemoteNodeAddress address, final boolean unilateral) {
		toUnregister.add(address.physicalLocation);
		return sessionMgr.connectionManager.getConnection(address, unilateral, userLabel);
	}

	protected TrRemoteConnection connectionWithUserLabel(final PhysicalNetworkLocation physicalLocation, final String label) {
		return connectionWithUserLabel(new RemoteNodeAddress(physicalLocation, null), false, label);
	}

	protected TrRemoteConnection connectionWithUserLabel(final PhysicalNetworkLocation physicalLocation, final boolean unilateral, final String label) {
		return connectionWithUserLabel(new RemoteNodeAddress(physicalLocation, null), unilateral, label);
	}

	protected TrRemoteConnection connectionWithUserLabel(final RemoteNodeAddress address, final String label) {
		return connectionWithUserLabel(address, false, label);
	}

	protected TrRemoteConnection connectionWithUserLabel(final RemoteNodeAddress address, final boolean unilateral, final String label) {
		toUnregister.add(address.physicalLocation);
		return sessionMgr.connectionManager.getConnection(address, unilateral, label);
	}

    public <T extends TrSession> T remoteSession(final Class<T> cls, final PhysicalNetworkLocation location) {
        return remoteSession(cls, connection(location));
    }

    public <T extends TrSession> T remoteSession(final Class<T> cls, final RemoteNodeAddress location) {
        return remoteSession(cls, connection(location));
    }

	public <T extends TrSession> T remoteSession(final Class<T> cls,
			final TrRemoteConnection conn) {
		return remoteSession(cls, conn, sessionId);
	}

	protected <T extends TrSession> T remoteSession(final Class<T> cls,
			final TrRemoteConnection conn,
			final int sessionId) {
