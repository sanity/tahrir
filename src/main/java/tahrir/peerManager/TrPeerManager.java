...
public class TrPeerManager {
	...
	public ConcurrentLinkedQueue<TrRemoteAddress> lastAttemptedRelays = new ConcurrentLinkedQueue<TrRemoteAddress>();
	...
	public Tuple2<TrRemoteAddress, RSAPublicKey> getPeerForAssimilation() {
		if (peers.isEmpty()) {
	...
	public void maintainance() {
		...
		if (config.assimilate && peers.size() < config.minPeers) {
			...
			if (peers.isEmpty()) {
				...
				final TrRemoteAddress peerForAssimilation = getPeerForAssimilation();

				as.startAssimilation(TrUtils.noopRunnable, , assimilateViaPublicKey, unilateral)
			} else {
